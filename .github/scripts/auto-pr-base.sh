...
# Get PR details
pr_json=$(curl -sSf -H "Authorization: Bearer ${GITHUB_TOKEN}" "${API_URL}/pulls/${PR_NUMBER}")
...
parents=$(curl -sSf -H "Authorization: Bearer ${GITHUB_TOKEN}" "${API_URL}/commits/${head_sha}")
first_parent_sha=$(echo "$parents" | jq -r '.parents[0].sha // empty')

candidate_branches=()
if [ -n "$first_parent_sha" ]; then
  branches_json=$(curl -sSf -H "Authorization: Bearer ${GITHUB_TOKEN}" "${API_URL}/commits/${first_parent_sha}/branches-where-head")
...
if [ ${#candidate_branches[@]} -eq 0 ]; then
  recent_branches=$(curl -sSf -H "Authorization: Bearer ${GITHUB_TOKEN}" "${API_URL}/branches?per_page=50")
...
curl -sSf -X PATCH -H "Authorization: Bearer ${GITHUB_TOKEN}" -H "Content-Type: application/json" -d "$patch" "${API_URL}/pulls/${PR_NUMBER}" >/dev/null

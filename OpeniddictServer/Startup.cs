	...
	public void ConfigureServices(IServiceCollection services)
	{
		...
		services.AddCors(options =>
		{
			options.AddPolicy("AllowAllOrigins",
				builder =>
				{
					builder
						.AllowCredentials()
						.WithOrigins("https://localhost:4200", "https://localhost:4204")
		...
		services.AddOpenIddict()
			...
			.AddServer(options =>
			{
				...
				// Keep encryption certificate for refresh tokens and authorization codes
				// but disable it specifically for access tokens so ResourceServer can validate them
				options.AddDevelopmentEncryptionCertificate()
					   .AddDevelopmentSigningCertificate();

				// Disable ONLY access token encryption (refresh tokens, auth codes still encrypted)
